...
fun Context.markThreadMessagesRead(threadId: Long) {
    ...
    val smsSelection = "${Sms.THREAD_ID}=? AND (${Sms.READ}=? OR ${Sms.SEEN}=?)"
    val smsArgs = arrayOf(id, "0", "0")
    ...
    val mmsSelection = "${Mms.THREAD_ID}=? AND (${Mms.READ}=? OR ${Mms.SEEN}=?)"
    val mmsArgs = arrayOf(id, "0", "0")
